...
from voc2012_tool.randomize_xml_annotation import *

if __name__ == '__main__':
    # 处理命令行输入参数
    parser = argparse.ArgumentParser()
    parser.description = ('此程序用于训练带有噪声的vgg模型，训练过程将会在控制台输出，可实时监测，训练过程将记录在文件vgg_[vgg_type]_train_log.csv\n' +
    ...
    parser.add_argument('ratio', help='噪声添加比例', type=float)
    ...
    # 备份正确的数据集
    if verbose:
        print('正在备份正确的数据集,请勿停止程序...')
    backup_annotations(dataset_path=voc2012_root)
    if verbose:
        print('数据集已经备份成功，备份目录：{}'.format(os.path.join(os.getcwd()), backup_dir))
    ...
    if verbose:
        print('模型训练结束，开始恢复数据集')
    restore_annotations(dataset_path=voc2012_root)
    if verbose:
        print('数据集恢复完毕，请手动删除备份文件，备份文件目录：{}'.format(os.path.join(os.getcwd()), backup_dir))
