...
def training(loader, vgg_model, criterion, optimizer, output_processor, label_int_map, verbose, device, use_gpu, ep):
    ...
    for batch_idx, sample_batched in enumerate(loader):
        ...
        sample_batched['images'] = sample_batched['images'].float().to(device)

        label = gen_BECloss_label(sample_batched['annotations']['object'], label_int_map)
        label = label.to(device)

        # 前向传播
        out = vgg_model(sample_batched['images'])
...
def train_model(vgg_type, voc2012_root, batch_size, epoch, lr, log_save_file, use_gpu=True, shuffle=True, verbose=True,
    ...
    # 载入以前训练的模型
    if prev_model != None:
        vgg_model = (torch.load(prev_model, map_location=torch.device('cpu')))
        vgg_model.eval()
        if isinstance(vgg_model, nn.DataParallel):  # 载入之前被存取为DataParallel的模型
            vgg_model = vgg_model._modules['module']
        if verbose:
            print('载入之前模型：{}'.format(prev_model))
    assert isinstance(vgg_model, my_VGG)  # 确认模型的类型为My_VGG
    ...
    if use_gpu:
        ...
        if verbose:
            ...
            print('所有GPU编号：{}'.format(ids))
        if torch.cuda.device_count() >= 1:
            vgg_model = nn.DataParallel(vgg_model, device_ids=ids)
            device = torch.device("cuda:{}".format(vgg_model.device_ids[0]))
    vgg_model.to(device)
    ...
    for ep in range(epoch):
        ...
        t2 = time.time()
        if verbose:
            print('此次训练耗时：{}s'.format(int(t2 - t1)))
        vl = validating(dataloader_val, vgg_model, criterion, optimizer, sigmoid, label_int_map, verbose, device,
                        use_gpu, ep)
        t3 = time.time()
        if verbose:
            print('此次验证耗时：{}s'.format(int(t3 - t2)))

        res_time = (t3 - t1) * (epoch - ep)
        print('预计耗时：{}m{}s'.format(round(res_time / 60), int(res_time % 60)))
        # 记录此次训练结果
        ...
        if vl < val_loss:
            if verbose:
                ...
                print('保存最优训练结果： vgg_{}_best_dict.pth'.format(vgg_type))
            torch.save(vgg_model.state_dict(), 'vgg_{}_best_dict.pth'.format(vgg_type))
            val_loss = vl
        # 保存最近一次训练出的模型
        if verbose:
